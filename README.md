...
TUI 支持直接使用中文输入法。用户输入会用带背景的全宽卡片区分，但仍逐字显示、不解析 Markdown；Assistant 内容按 Markdown 美化，系统提示和错误信息保持纯文本。上下键始终用于输入历史，不承担多行输入的垂直光标移动；粘贴的多行文本仍可原样发送。`/help` 和 `/chat` 不是本地命令，会作为普通文本发送给模型。当前不支持 HTML、远程图片、Mermaid、流式输出、多会话管理、历史搜索、上下文压缩、写操作工具或请求级模型切换。
