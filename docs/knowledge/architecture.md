...
这是一个基于 Python 3.11 的轻量模型调用项目，同时提供无状态 FastAPI HTTP 和可恢复单会话的 Textual TUI。两个入口共享 Chat Runtime；Runtime 通过 Provider Turn 选择阿里云或 DeepSeek，并自动编排根目录白名单只读工具；根目录 `main.py` 只保留 Uvicorn 兼容入口。
...
- `app/services/llm/contracts.py`：中立角色/消息、ModelStep、Provider Turn 协议和共享异常。
- `app/services/llm/factory.py`：解析环境并创建当前 Provider。
- `app/services/llm/http_client.py`：共享异步 JSON POST、超时、I/O 边界事件记录和脱敏错误处理。
- `app/services/llm/aliyun.py`：阿里云 Responses Turn、Function Call 续接和文本提取。
- `app/services/llm/deepseek.py`：DeepSeek Chat Completions Turn、assistant/tool 续接和文本提取。
...
- `app/tui/application.py`：终端输入与历史、用户消息卡片、Assistant Markdown 展示、请求活动 Timer、状态、耗时和取消。
...
POST /chat
  ...
  -> bounded loop calls Turn.next()
     -> Provider builds protocol-specific payload and calls shared HTTPX
...
DeepSeek Turn 把 assistant `tool_calls` 和对应 `role=tool/tool_call_id` 结果加入 messages，并保留工具调用时的 `reasoning_content`。阿里云 Turn 把每个 `function_call` 与对应 `function_call_output` 紧邻加入 input。两家结构都在 Provider 内转换为中立 ToolCall/ToolResult。
...
python -m app.tui
  ...
  -> request-scoped 100 ms Timer renders spinner, thinking state and live elapsed time
...
TUI 不读取 Provider 专属密钥变量，不解析 Provider JSON，也不逐次确认只读工具；状态信息和实际调用共用工厂配置规则。工具轨迹只存在于当前 Turn，Session 仍只提交最终 user/assistant 消息。Assistant 的 Markdown 只在 `RichLog` 展示时构造为 Rich Renderable，恢复历史和新响应复用同一路径；用户消息使用 Rich Panel 增加背景和边框但不解析 Markdown，系统和错误仍按纯文本显示，Session 与后续模型请求继续使用未经改写的原文。请求活动 Timer 只展示应用固定文案和单调时钟耗时，不读取 reasoning、Token 或 Provider 状态，并在成功、错误、取消和退出时按请求代次停止。HTTP `/chat` 不加载或修改 TUI 会话文件。
