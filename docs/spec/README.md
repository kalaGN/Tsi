...
- [MCP 客户端需求](20260922-MCP客户端-需求.md)：在 Web 与 TUI 连接显式配置的 stdio/Streamable HTTP Server，发现工具并逐次审批调用；对应[设计](../plan/20260922-MCP客户端-设计.md)和[任务](../tasks/20260922-MCP客户端-任务.md)。

- [Web 项目与会话分组需求](20260922-Web项目与会话分组-需求.md)：左侧按项目组织 Web 会话、配置本地项目路径，并让 Workspace/规则随会话切换；已完成。
- [个性化系统提示词需求](20260921-个性化系统提示词-需求.md)：设置页新增“个性化”菜单，本机持久化 Web 全会话共享的自定义系统提示词；已完成。
- [上下文压缩优化需求](20260920-上下文压缩优化-需求.md)：补齐逐步预算、有界摘要、取消传播、v3 迁移和界面占比，保留工具副作用及既有日志策略；审核修订完成，技术设计草案已补齐。
- [Web 设置与统计需求](20260920-Web设置与统计-需求.md)：将设置弹窗改为带左侧菜单的独立页面，并增加不含对话正文的本地使用统计；已完成。
- [模型响应性能优化需求](20260920-模型响应性能优化-需求.md)：复用单次请求连接、记录首响应与首文本耗时，并增加有界批量只读工具；已完成。
- [移除 HTTP 对话接口需求](20260919-移除HTTP对话接口-需求.md)：删除对外 `POST /chat` 接口、路由层、错误映射和契约测试，只保留 Web UI 与 TUI 两个对话入口；已完成。
- [Web 网络搜索需求](20260919-Web网络搜索-需求.md)：为 HTTP 与本机 Web 增加固定 Serper 上游、有界结果和按需披露的网络搜索；已完成。其中 HTTP 入口已在同日后续变更中移除，当前以 Web 注册为准。
- [Web 工作区写入需求](20260919-Web工作区写入-需求.md)：为本机 Web 对话增加按需写工具、逐次 Diff 审批、取消清理和严格请求绑定；已完成。
- [Web 多会话需求](20260918-Web多会话-需求.md)：让 Web 页面与后端支持独立持久化的多会话、新建、切换、重命名、删除和旧数据迁移；已完成。
- [Web UI 与 macOS 桌面应用需求](20260918-mac桌面应用-需求.md)：参考 DSH 信息架构，先为共享 Runtime 增加本机三栏 Web UI、流式对话和只读 Workspace；Web 阶段已完成，macOS 壳待确认。
- [Agent 评测系统需求](20260911-Agent评测系统-需求.md)：为当前单 Agent Runtime 增加离线回放、显式真实模型运行、结构化轨迹、确定性评分、可选 LLM Judge、报告和基线回归检测；已完成。
- [TUI 应用职责收敛需求](20260909-TUI应用职责收敛-需求.md)：从主应用收敛启动装配、请求生命周期和模型选择责任，保持扁平 TUI 目录和现有行为；已完成。
- [TUI 模型选择持久化需求](20260909-TUI模型选择持久化-需求.md)：记住最近一次成功切换的供应商与模型，并在下次 TUI 启动时安全恢复；已完成。
- [TUI 单文件删除工具需求](20260909-TUI单文件删除工具-需求.md)：增加仅限 TUI、逐次审批、哈希防冲突且可撤销的单个文本文件删除能力；已完成。
- [TUI 记忆管理需求](20260909-TUI记忆管理-需求.md)：自动摘要、上下文淘汰和长期用户偏好；已完成。
- [TUI 安全 Git 写工具](20260907-TUI安全Git写工具-需求.md)：增加逐次审批的指定路径暂存、中文提交和既有上游推送工具；已完成。
- [TUI 意图工具组](20260907-TUI意图工具组-需求.md)：按请求通过模型意图渐进激活工具组，减少固定工具 Schema 的 Token 和误调用；已完成。
- [TUI 模型切换](20260907-TUI模型切换-需求.md)：通过 Codex 风格 `/model` 候选列表和键盘操作切换当前 TUI 进程的供应商与模型；已完成。
