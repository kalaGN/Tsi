...
- [Token 消耗统计](20260906-Token消耗统计-需求.md)：解析两家 Provider 的真实 usage，累计工具循环全部模型步骤，并在 TUI 和日志中展示；已完成。
- [运行与测试日志隔离](20260906-运行与测试日志隔离-需求.md)：将 HTTP/TUI 真实使用日志与 Pytest 日志写入不同固定目录；已完成。
- [TUI 美元符号引用技能](20260905-TUI美元符号引用技能-需求.md)：通过 `$技能名` 候选补全并在单次请求快照中显式加载 Skill；已完成。
- [TUI 命令预览](20260905-TUI命令预览-需求.md)：输入前缀查看候选、键盘选择补全和 Esc 关闭。
