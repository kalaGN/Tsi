...
def test_harness_fingerprint_is_stable_and_changes_with_agents(tmp_path):
    ...
    first = build_harness_fingerprint(tmp_path, create_default_registry().definitions, ModelBudget(), DEFAULT_TOOL_LOOP_LIMITS)
    second = build_harness_fingerprint(tmp_path, create_default_registry().definitions, ModelBudget(), DEFAULT_TOOL_LOOP_LIMITS)
    (tmp_path / "AGENTS.md").write_text("规则二", encoding="utf-8")
    changed = build_harness_fingerprint(tmp_path, create_default_registry().definitions, ModelBudget(), DEFAULT_TOOL_LOOP_LIMITS)
